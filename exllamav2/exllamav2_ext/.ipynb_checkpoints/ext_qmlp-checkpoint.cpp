...
(
    ...
    float norm_epsilon,
    torch::Tensor pre_ff_layernorm,
    torch::Tensor pre_ff_layernorm_bias,
    bool pre_ff_layernorm_is_rms,
    float pre_ff_norm_epsilon,
    torch::Tensor post_ff_layernorm,
    torch::Tensor post_ff_layernorm_bias,
    bool post_ff_layernorm_is_rms,
    float post_ff_norm_epsilon,
    uintptr_t q_gate,
    uintptr_t q_up,
    uintptr_t q_down,
    ...
    torch::Tensor temp_state_pre_ff,
    torch::Tensor temp_state_post_ff,
    int max_rows,
    bool act_gelu,
    bool has_residual
)
{
    ...
    if (pre_ff_layernorm.defined()) {
        TORCH_CHECK_DTYPE_OPT(pre_ff_layernorm, kHalf);
    }

    if (post_ff_layernorm.defined()) {
        TORCH_CHECK_DTYPE_OPT(post_ff_layernorm, kHalf);
    }

    // todo: check shapes of pre_ff_layernorm and post_ff_layernorm if provided
    ...
    (
        ...
        layernorm_is_rms,
        norm_epsilon,
        (half*) pre_ff_layernorm.is_meta() ? NULL : (half*) pre_ff_layernorm.data_ptr(),
        (half*) pre_ff_layernorm_bias.is_meta() ? NULL : (half*) pre_ff_layernorm_bias.data_ptr(),
        pre_ff_layernorm_is_rms,
        pre_ff_norm_epsilon,
        (half*) post_ff_layernorm.is_meta() ? NULL : (half*) post_ff_layernorm.data_ptr(),
        (half*) post_ff_layernorm_bias.is_meta() ? NULL : (half*) post_ff_layernorm_bias.data_ptr(),
        post_ff_layernorm_is_rms,
        post_ff_norm_epsilon,
        qm_gate,
        qm_up,
        qm_down,
        ...
        (half*) temp_state_pre_ff.data_ptr(),
        (half*) temp_state_post_ff.data_ptr(),
        max_rows,
        act_gelu,
        has_residual
    );
