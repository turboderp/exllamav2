...
(
    ...
    float _norm_epsilon,
    half* _pre_ff_layernorm,
    half* _pre_ff_layernorm_bias,
    bool _pre_ff_layernorm_is_rms,
    float _pre_ff_norm_epsilon,
    half* _post_ff_layernorm,
    half* _post_ff_layernorm_bias,
    bool _post_ff_layernorm_is_rms,
    float _post_ff_norm_epsilon,
    QMatrix* _gate,
    QMatrix* _up,
    QMatrix* _down,
    half* _temp_state,
    ...
    half* _temp_b,
    half* _temp_dq,
    half* _temp_state_pre_ff,
    half* _temp_state_post_ff,
    int _max_rows,
    bool _act_gelu,
    bool _has_residual
):
...
pre_ff_layernorm(_pre_ff_layernorm),
    pre_ff_layernorm_bias(_pre_ff_layernorm_bias),
    pre_ff_layernorm_is_rms(_pre_ff_layernorm_is_rms),
    pre_ff_norm_epsilon(_pre_ff_norm_epsilon),
post_ff_layernorm(_post_ff_layernorm),
    post_ff_layernorm_bias(_post_ff_layernorm_bias),
    post_ff_layernorm_is_rms(_post_ff_layernorm_is_rms),
    post_ff_norm_epsilon(_post_ff_norm_epsilon),
    gate(_gate),
    up(_up),
    down(_down),
    ...
    temp_b(_temp_b),
    temp_dq(_temp_dq),
    temp_state_pre_ff(_temp_state_pre_ff),
    temp_state_post_ff(_temp_state_post_ff),
    max_rows(_max_rows),
    act_gelu(_act_gelu),
...
)
{
    ...
    // pre_feedforward_layernorm if provided

    if (pre_ff_layernorm)
    {
        if (pre_ff_layernorm_is_rms)
            rms_norm_cuda(x, pre_ff_layernorm, temp_state_pre_ff, pre_ff_norm_epsilon, rows, columns);
        else
            layer_norm_cuda(x, pre_ff_layernorm, pre_ff_layernorm_bias, temp_state_pre_ff, pre_ff_norm_epsilon, rows, columns);
        norm_state = temp_state_pre_ff;
    }
    ...
    // post_feedforward_layernorm if provided

    if (post_ff_layernorm)
    {
        if (post_ff_layernorm_is_rms)
            rms_norm_cuda(x, post_ff_layernorm, temp_state_post_ff, post_ff_norm_epsilon, rows, columns);
        else
            layer_norm_cuda(x, post_ff_layernorm, post_ff_layernorm_bias, temp_state_post_ff, post_ff_norm_epsilon, rows, columns);
        x = temp_state_post_ff;
    }
}
