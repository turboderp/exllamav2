...
)
{
    ...
    TORCH_CHECK_DTYPE(q_perm, kShort);
    TORCH_CHECK_DTYPE(q_invperm, kShort);
    TORCH_CHECK_DTYPE(q_scale, kInt);
    TORCH_CHECK_DTYPE(q_scale_max, kHalf);
...
)
{
    ...
    int rows = a.size(0);
    bool reconstruc = (rows > MAX_Q_GEMM_ROWS);
