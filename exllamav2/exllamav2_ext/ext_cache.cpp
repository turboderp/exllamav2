...
void fp16_to_q4(torch::Tensor in_tensor, torch::Tensor out_tensor, torch::Tensor scales, int batch_size, int offset, int width)
{
    ...
    TORCH_CHECK_SHAPES(in_tensor, 1, out_tensor, 1, 1);
    TORCH_CHECK_SHAPES(in_tensor, 2, out_tensor, 2, 1);
    TORCH_CHECK_SHAPES(in_tensor, 3, out_tensor, 3, 2);
...
void q4_to_fp16(torch::Tensor in_tensor, torch::Tensor scales, torch::Tensor out_tensor, int batch_size, int offset, int width)
{
    ...
    TORCH_CHECK_SHAPES(out_tensor, 0, in_tensor, 0, 1);
    TORCH_CHECK_SHAPES(out_tensor, 1, in_tensor, 1, 1);
    TORCH_CHECK_SHAPES(out_tensor, 2, in_tensor, 2, 1);
    TORCH_CHECK_SHAPES(out_tensor, 3, in_tensor, 3, 2);

    int stride = out_tensor.size(1) * out_tensor.size(2) * out_tensor.size(3);
    int height = batch_size;

    int tsize = out_tensor.size(2) * out_tensor.size(3);
    offset *= tsize;
    width *= tsize;

    array_q4_to_fp16_cuda
    (
        (const unsigned char*) in_tensor.data_ptr(),
