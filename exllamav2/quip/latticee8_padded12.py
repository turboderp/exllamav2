...
def get_abs_grid(self):
    intr = torch.arange(-4, 4)
    d8 = torch.cartesian_prod(*[intr] * self._E8P_CODESZ).float() + 1 / 2
...
class QuantizedE8P12Linear:
    ...
    def __init__(self, device):
        self.device = device
        self.grid_abs = get_abs_grid().to(torch.float16).to(self.device)
    ...
    def forward(self,
        ...
        x = input.view(-1, n * self._E8P_CODESZ).to(torch.float32)
        if rescale_WH:
            x /= scaleWH
        x = x * SU
        ...
        if x.size(0) < 3:
            ...
            W_decompressed = torch.zeros(m, n*self._E8P_CODESZ, device=self.device, dtype=torch.float16)
